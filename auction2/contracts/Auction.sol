...
contract Auction {
    ...
    // Register new Bidder
    function register(address _account, uint8 _token) public check(State.CREATED, true) { 
    ...
    // Start the session.
    function startSession() public check(State.CREATED, true){
        state = State.STARTED;
    }
    

    
    function bid(uint8 _price) public check(State.STARTED, false){
    ...
    function announce() public check(State.STARTED, true){
    ...
    function getDeposit() public check(State.CLOSING , false ) {
	   ...
	   address bidderAddr = msg.sender; 
       if(msg.sender == currentWinner ) revert();
       IBidder storage currentBidder = bidders[bidderAddr];
       currentBidder.token += currentBidder.deposit;
       totalDeposit -= currentBidder.deposit;
       currentBidder.deposit = 0;
       if(totalDeposit == bidders[currentWinner].deposit) { totalDeposit = 0; }
    ...
    // modifier
    modifier check(State requireState, bool auctioneerRequire)
    { 
        require(state == requireState);
        if(auctioneerRequire) {
